...
describe("OpenAPI documentation", () => {
  test("serves Scalar and a documented bearer-auth API contract", async () => {
    ...
    try {
      ...
      expect(document.paths["/references"]?.get?.security).toEqual([
        { bearerAuth: [] },
      ]);
      expect(document.paths["/references/upload"]?.post?.security).toEqual([
        { bearerAuth: [] },
      ]);
      expect(document.paths["/references/{id}/file-url"]?.get?.security).toEqual([
        { bearerAuth: [] },
      ]);
      expect(document.paths["/action-requests/{id}/references"]?.get?.security).toEqual([
        { bearerAuth: [] },
      ]);
      expect(document.paths["/action-requests/{id}/references"]?.post?.security).toEqual([
        { bearerAuth: [] },
      ]);
      expect(
